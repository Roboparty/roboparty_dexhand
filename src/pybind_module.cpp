...
PYBIND11_MODULE(dexhand_py, m) {
    m.doc() = "Dexterous Hand Driver Python SDK (roboparty_dexhand)";

    // Enums
    py::enum_<HandCommType>(m, "HandCommType")
        .value("CANFD", HandCommType::CANFD)
        .value("ETHERCAT", HandCommType::ETHERCAT)
        .value("RS485", HandCommType::RS485)
        .export_values();

    py::enum_<HandModel>(m, "HandModel")
        .value("LHANDPRO_6DOF", HAND_LHANDPRO_6DOF)
        .value("LHANDPRO_16DOF", HAND_LHANDPRO_16DOF)
        .export_values();

    // Only bind the base class — Python never sees vendor subclasses.
    py::class_<HandDriver, std::shared_ptr<HandDriver>>(m, "HandDriver")
        .def_static("create_hand", &HandDriver::create_hand,
                    ...
                    py::arg("canfd_nom_baudrate") = 1000000,
                    py::arg("canfd_dat_baudrate") = 5000000)
        // Lifecycle
        .def("init_hand", &HandDriver::init_hand,
             py::arg("enable_motors") = true,
             py::arg("home_motors") = true,
             py::arg("home_wait_time") = 5.0)
