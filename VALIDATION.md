...
的一个，并按现场分配修改 `NODE_ID`（有效范围为 `1..127`）。输出必须显示
...
手本体重新上电后，在新的独立 shell 中执行以下完整命令块确认持久化配置：
...
按实际接口和 node ID 修改这两个导出值。`show` 输出的六个轴 raw value 必须
全部为 `200`；任何一个轴不是 `200` 都不能进入运动验收。

## Python API 识别

在新的独立 shell 中确认已安装 Python API 的公开模型数值：
...
运动授权。下面命令块会在 Python 中要求操作员精确输入 `MOVE`；其他任何输入均
会中止，且不会发送运动命令。可直接在新的 shell 中执行，必要时将默认接口和
node ID 改为现场分配值：

```bash
source /opt/roboparty/setup.bash
export CAN_INTERFACE="${CAN_INTERFACE:-can3}"
...
try:
    ...
    if input("Confirm known zero/reference and type MOVE: ") != "MOVE":
        raise RuntimeError("motion aborted: confirmation was not MOVE")
...
脚本只在 `init_hand(True, False, 0.0)` 成功、健康检查通过、活动 DOF 为 `6`、
且关节 `1..6` 报警全为 `0` 后，才允许免回零运动。`total` DOF 可能为 `11`，
验收以活动 DOF 为准。清理阶段会在已初始化时尝试 `stop_motors(0)`，在已启用
免回零时恢复 `set_move_no_home(0)`，并且只调用一次 `deinit_hand()`；清理失败
不会覆盖先前发生的主错误。
